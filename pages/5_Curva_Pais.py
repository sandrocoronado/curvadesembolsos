...
def run():
    ...
    st.title("Desembolsos Por Paises📊")
    ...
    if uploaded_file:
        ...
        # Suponiendo que 'result_df' es tu DataFrame y tiene las columnas 'Paises' y 'APODO'
        combined_series = result_df['Pais'] 

        # Usando esta serie combinada en el selectbox
        selected_combined = st.selectbox('Selecciona el Proyecto:', combined_series.unique())

        # Extraemos el valor del texto seleccionado para filtrar el DataFrame
        ...
        df_monto_anual = filtered_df.groupby('Ano')["Monto"].mean().reset_index()
        df_monto_acumulado_anual = df_monto_anual['Monto'].mean()

        # Calcular Porcentaje del Monto de forma acumulativa
        # Calcular la suma de los valores únicos de AporteFonplata
        aporte_total = filtered_df['AporteFonplata'].unique().sum()

        # Calcular el porcentaje del monto de forma anual y acumulativa
